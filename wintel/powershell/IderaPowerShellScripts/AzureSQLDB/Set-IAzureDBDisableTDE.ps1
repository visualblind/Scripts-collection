<#
	.SYNOPSIS
		Set-IAzureDBDisableTDE
	.DESCRIPTION
		Disable Azure SQL DB transparent data encryption
	...
	.PARAMETER state
		Disable TDE
	.EXAMPLE
        .\Set-IAzureDBDisableTDE -resourceGroupName MyResource -serverName MyServer -databaseName AdventureWorksLTv12
    .INPUTS
	.OUTPUTS
    .NOTES
	.LINK
...
process {
	try {
        Set-AzureRmSqlDatabaseTransparentDataEncryption -ResourceGroupName $resourceGroupName -ServerName $serverName -DatabaseName $databaseName -State Disabled -Confirm
	}
	catch [Exception] {
		Write-Error $Error[0]
		$err = $_.Exception
