...
then
  ...
  if [[ $? > 0 ]]
  then
    if [[ $(find /etc/apt/* -name *.list | xargs cat | grep -P -c "^deb http://ftp.[A-Za-z0-9]*.debian.org/debian buster main") -eq 0 ]]
	then
...
then
  ...
  if [[ $? > 0 ]]
  then
    if [[ $(find /etc/apt/* -name *.list | xargs cat | grep -c "^deb http://security.debian.org/debian-security buster/updates main") -eq 0 ]]
	then
	  echo deb http://security.debian.org/debian-security buster/updates main >>/etc/apt/sources.list.d/glennr-install-script.list || abort
	  apt-get update
...
then
  ...
  echo "                                   This script is made for Debian Buster"
  echo ""
