...
then
  ...
  if [[ $? > 0 ]]
  then
    if [[ $(find /etc/apt/* -name *.list | xargs cat | grep -c "^deb http://security.debian.org/debian-security stretch/updates main") -eq 0 ]]
	then
	  echo deb http://security.debian.org/debian-security stretch/updates main >>/etc/apt/sources.list.d/glennr-install-script.list || abort
	  apt-get update
...
if [[ $OS_NAME != "stretch" && $OS_DESC != "MX 18"* ]]
then
  echo -e "${RED}################################################################################################################${RESET}"
  echo ""
  echo "                                    You seem to have ${OS_DESC}"
  echo "                                   This script is made for Debian Stretch"
  echo ""
...
then
  ...
  then
	...
	if [[ $? > 0 ]]
	then
	  if [[ $(find /etc/apt/* -name *.list | xargs cat | grep -c "^deb http://ftp.nl.debian.org/debian stretch main") -eq 0 ]]
	  then
		echo deb http://ftp.nl.debian.org/debian stretch main >> /etc/apt/sources.list.d/glennr-install-script.list || abort
		apt-get update
		apt-get install openjdk-8-jre-headless -y
