...
then
  ...
  if [[ $? > 0 ]]
  then
    if [[ $(find /etc/apt/* -name *.list | xargs cat | grep -P -c "^deb http://[A-Za-z0-9]*.archive.ubuntu.com/ubuntu disco main") -eq 0 ]]
    then
...
then
  ...
  if [[ $? > 0 ]]
  then
    if [[ $(find /etc/apt/* -name *.list | xargs cat | grep -c "^deb http://security.ubuntu.com/ubuntu disco-security main") -eq 0 ]]
	then
...
then
  ...
  if [[ $? > 0 ]]
  then
    if [[ $(find /etc/apt/* -name *.list | xargs cat | grep -P -c "^deb http://[A-Za-z0-9]*.archive.ubuntu.com/ubuntu disco main universe") -eq 0 ]]
	then
...
then
  ...
  echo "                                   This script is made for Ubuntu 19.04"
  echo ""
...
then
  ...
  echo -e "${RED}#    ${RESET}32 bit MongoDB isn't supported on Ubuntu 19.04    ${RED}#${RESET}"
  echo -e "${RED}#        ${RESET}I recommend using Ubuntu 19.04 64 bit!        ${RED}#${RESET}"
  echo -e "${RED}#                                                      #${RESET}"
