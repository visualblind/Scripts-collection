...
then
  ...
  then
	...
	echo ""
    sleep 2
    if [[ $(find /etc/apt/* -name *.list | xargs cat | grep -P -c "^deb http://ftp.debian.org/debian jessie-backports main") -eq 0 ]]
    then
	  echo deb http://ftp.debian.org/debian jessie-backports main >>/etc/apt/sources.list.d/glennr-install-script.list || abort
	  apt-get update || abort
    fi
