<#
	...
	.PARAMETER dbName
		Database name
	.EXAMPLE
		.\Set-ISqlAutoTuningOff -serverInstance MyServer -dbName SMOTestDB
	.INPUTS
	.OUTPUTS
		Sets automatic tuning off for specified database
	.NOTES
...
process {
	try {
		...
		if ($ver -ge 14) {
			Invoke-SqlCmd -ServerInstance $serverInstance -Query "ALTER DATABASE $dbName SET AUTOMATIC_TUNING ( FORCE_LAST_GOOD_PLAN = OFF );"
