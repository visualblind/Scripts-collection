...
# UniFi Controller 5.10.23 auto installation script.
# OS       | Xenial
# Version  | 3.8.8
...
if [ $(cat /root/.bashrc | grep "^export PATH=" | grep -c "/sbin") -eq 0 ]
then
  if [[ $(cat /root/.bashrc | grep "^export PATH=") ]]
  then
...
then
  ...
  if [[ $? > 0 ]]
  then
    if [[ $(find /etc/apt/* -name *.list | xargs cat | grep -P -c "^deb http://[A-Za-z0-9]*.archive.ubuntu.com/ubuntu xenial main") -eq 0 ]]
    then
...
then
  ...
  if [[ $? > 0 ]]
  then
    if [[ $(find /etc/apt/* -name *.list | xargs cat | grep -c "^deb http://security.ubuntu.com/ubuntu xenial-security main") -eq 0 ]]
	then
...
then
  ...
  if [[ $? > 0 ]]
  then
    if [[ $(find /etc/apt/* -name *.list | xargs cat | grep -c "^deb http://security.ubuntu.com/ubuntu xenial main universe") -eq 0 ]]
	then
...
JAVA12=$(dpkg -l | grep ^ii | grep -c "openjdk-12\|oracle-java12")
...
if [[ $OS_RELEASE != "16.04" && $OS_DESC != "Linux Mint 18"* ]]
then
  echo -e "${RED}################################################################################################################${RESET}"
  echo ""
...
then
  ...
  then
    ...
    echo -e "${GREEN}#       ${RESET}Installing MongoDB for 32 bit systems!         ${GREEN}#${RESET}"
    echo -e "${GREEN}#                                                      #${RESET}"
    echo -e "${GREEN}########################################################${RESET}"
    echo ""
	echo ""
    ...
    if [[ $? > 0 ]]
    then
      rm /etc/apt/sources.list.d/mongodb-org-2.6.list
      if [ $(dpkg-query -W -f='${Status}' mongodb-server 2>/dev/null | grep -c "ok installed") -eq 0 ]
      then
        apt-get install mongodb-server -y
        if [[ $? > 0 ]]
        then
          if [[ $(find /etc/apt/* -name *.list | xargs cat | grep -c "^deb http://[A-Za-z0-9]*.archive.ubuntu.com/ubuntu xenial main universe") -eq 0 ]]
          then
            echo deb http://nl.archive.ubuntu.com/ubuntu xenial main universe >>/etc/apt/sources.list.d/glennr-install-script.list || abort
            apt-get update
	        apt-get install mongodb-server || abort
          fi
        fi
      fi
      if [ $(dpkg-query -W -f='${Status}' mongodb-clients 2>/dev/null | grep -c "ok installed") -eq 0 ]
      then
        ...
        then
          ...
          then
            echo deb http://nl.archive.ubuntu.com/ubuntu xenial main universe >>/etc/apt/sources.list.d/glennr-install-script.list || abort
            apt-get update
	       apt-get install mongodb-clients || abort
          fi
        fi
      fi
    fi
...
then
  ...
  then
	...
	if [[ $? > 0 ]]
	then
	  if [[ $(find /etc/apt/* -name *.list | xargs cat | grep -c "^deb http://ppa.launchpad.net/openjdk-r/ppa/ubuntu xenial main") -eq 0 ]]
	  then
...
if [[ $JAVA9 -eq 1 || $JAVA10 -eq 1 || $JAVA11 -eq 1 || $JAVA12 -eq 1 ]]
then
...
then
  ...
  echo ""
  sleep 2
  if [[ $(find /etc/apt/* -name *.list | xargs cat | grep -P -c "^deb http://[A-Za-z0-9]*.archive.ubuntu.com/ubuntu xenial main universe") -eq 0 ]]
  then
    echo deb http://nl.archive.ubuntu.com/ubuntu xenial main universe >>/etc/apt/sources.list.d/glennr-install-script.list || abort
	apt-get update
  fi
...
echo -e "${GREEN}#         ${RESET}Installing UniFi Controller 5.10.23!         ${GREEN}#${RESET}"
echo -e "${GREEN}#                                                      #${RESET}"
echo -e "${GREEN}########################################################${RESET}"
echo ""
sleep 2
...
wget https://dl.ui.com/unifi/5.10.23/unifi_sysvinit_all.deb || abort
...
then
  ...
  echo ""
  echo ""
  echo -e "${GREEN}#${RESET} UniFi SDN Controller 5.10.23 has been installed successfully"
  ...
  echo ""
  echo ""
  echo " Failed to successfully install UniFi SDN Controller 5.10.23"
  echo ""
