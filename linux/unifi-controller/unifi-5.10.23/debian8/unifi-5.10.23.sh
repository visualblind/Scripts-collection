...
then
  ...
  if [ $(dpkg-query -W -f='${Status}' libjpeg-turbo8 2>/dev/null | grep -c "ok installed") -eq 0 ]
  then
	wget http://security.ubuntu.com/ubuntu/pool/main/libj/libjpeg-turbo/libjpeg-turbo8_1.3.0-0ubuntu2.1_amd64.deb -O /tmp/libjpeg-turbo8_1.3.0-0ubuntu2.1_amd64.deb || abort
	dpkg -i /tmp/libjpeg-turbo8_1.3.0-0ubuntu2.1_amd64.deb || abort
	rm /tmp/libjpeg-turbo8_1.3.0-0ubuntu2.1_amd64.deb
  fi
  if [ $(dpkg-query -W -f='${Status}' libjpeg8 2>/dev/null | grep -c "ok installed") -eq 0 ]
  then
    wget http://nl.archive.ubuntu.com/ubuntu/pool/main/libj/libjpeg8-empty/libjpeg8_8c-2ubuntu8_amd64.deb -O /tmp/libjpeg8_8c-2ubuntu8_amd64.deb || abort
	dpkg -i /tmp/libjpeg8_8c-2ubuntu8_amd64.deb || abort
	rm /tmp/libjpeg8_8c-2ubuntu8_amd64.deb
  fi
  ...
  then
    ...
    then
      ...
      if [[ $? > 0 ]]
      then
        if [[ $(find /etc/apt/* -name *.list | xargs cat | grep -P -c "^deb http://archive.debian.org/debian jessie-backports main") -eq 0 ]]
        then
...
sleep 2
apt-get update
apt-get install binutils ca-certificates-java java-common -y --force-yes
apt-get install jsvc libcommons-daemon-java -y --force-yes
if [[ $? > 0 ]]
then
  clear
  echo -e "${RED}########################################################${RESET}"
