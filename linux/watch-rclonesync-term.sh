#!/usr/bin/env bash
set -e
gnome-terminal --tab --title='gcrypt-usmba' -- watch -n 1 tail -n 20 /media/visualblind/rclone/root/root/.config/rclone/log/gcrypt-usmba.log && \
...
gnome-terminal --tab --title='up gdrive-servercopy' -- tail -fn 40 /media/visualblind/rclone/root/root/.config/rclone/log/upload-gdrive-servercopy.log && \
gnome-terminal --tab --title='up gcrypt-p0ds0smb' -- tail -fn 400 /media/visualblind/rclone/root/root/.config/rclone/log/upload-gcrypt-usmba-p0ds0smb.log
