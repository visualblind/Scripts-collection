...
// PREVENTIPV6LEAK = Prevent IPv6 Leak option
//                   Create two routes in the ipv6 routing table
//                   That points to TUNDEV interface.
...
//Sbg PATCH
//use TUNIDX environment variable
//only in case it is not defined,
//try to use the adapter friendly name
       var tundevid = env("TUNIDX");
       if ( !(tundevid > 0) ) {
               tundevid = getInterfaceId(env("TUNDEV"));
       }
//Sbg PATCH END
...
//Sbg PATCH
    ///////////////////////////////////////////////
    //Do not set DNS, our program will manage them
    ///////////////////////////////////////////////
...
//Patch
    if (env("PREVENTIPV6LEAK")) {
        exec("netsh interface ipv6 set address \"" + tundevid + "\" " +
			"FD00:1234::1 store=active");
		exec("netsh interface ipv6 add route 0000::/1 " + 
            " \"" + tundevid + "\" store=active");
		exec("netsh interface ipv6 add route 8000::/1 " + 
            " \"" + tundevid + "\" store=active");
    }
//Patch END
	break;
...
//Patch
    if (env("PREVENTIPV6LEAK")) {
		exec("netsh interface ipv6 delete route 0000::/1 " + 
            " \"" + tundevid + "\" store=active");
